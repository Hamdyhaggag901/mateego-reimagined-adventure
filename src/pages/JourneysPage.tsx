...
import { fetchTrips, Trip } from "@/services/tripsApi";
...
export default function JourneysPage() {
  ...
  const filters = ["All", ...Array.from(new Set(trips.map(() => "").filter(Boolean)))];
  const filtered = filter === "All" ? trips : trips;
          ...
          {filtered.map((j) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                {j.description && (
                  <p className="text-sm text-muted-foreground font-body mb-4">{j.description}</p>
                )}
